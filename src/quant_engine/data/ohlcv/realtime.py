...
class OHLCVDataHandler(RealTimeDataHandler):
    ...
    def _backfill_to_target(self, *, target_ts: int) -> None:
        ...
        if last_ts is None:
            ...
            if bars is None or bars <= 0:
                throttle_id = throttle_key("ohlcv.backfill.no_lookback", type(self).__name__, self.symbol, self.interval_ms)
    ...
    def _backfill_from_source(self, *, start_ts: int, end_ts: int, target_ts: int) -> int:
        ...
        if worker is None:
            throttle_id = throttle_key("ohlcv.backfill.no_worker", type(self).__name__, self.symbol, self.interval_ms)
            if log_throttle(throttle_id, 60.0):
                log_debug(
                    self._logger,
        ...
        if not callable(backfill):
            throttle_id = throttle_key("ohlcv.backfill.no_worker_method", type(self).__name__, self.symbol, self.interval_ms)
            if log_throttle(throttle_id, 60.0):
                log_debug(
                    self._logger,
