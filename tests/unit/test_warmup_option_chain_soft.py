...
class _FeatureExtractor:
    def __init__(self) -> None:
        self.required_windows = {"ohlcv": 2, "option_chain": 50}
        self.warmup_steps = 1
...
def _build_engine(*, data_root: Path) -> StrategyEngine:
...
def test_warmup_option_chain_warns_but_does_not_raise(caplog: pytest.LogCaptureFixture) -> None:
    data_root = Path(__file__).resolve().parents[1] / "resources"
    engine = _build_engine(data_root=data_root)
