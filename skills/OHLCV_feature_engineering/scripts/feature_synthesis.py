...
def synthesize(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if mode == "reversal":
        ...
        add(_base_spec(
            ...
            expression="log(close_t / anchor_t), where typical_price_t=((high_t+low_t+close_t)/3), refresh_t=min(refresh_cap, quote_asset_volume_t/(sum(quote_asset_volume_i, i=t-w_refresh+1..t)+eps)), and anchor_t=(1-refresh_t)*anchor_{t-1}+refresh_t*typical_price_t with anchor_{t0}=typical_price_{t0} at the first valid bar t0",
            inputs=["high_t", "low_t", "close_t", "quote_asset_volume_t", "quote_asset_volume_i"],
            parameters={"refresh_window_bars": 32, "refresh_cap": 0.20},
            parsed=parsed,
        ...
        add(_base_spec(
            ...
            parameters={"refresh_window_bars": 32, "z_window_bars": 64, "refresh_cap": 0.20},
            parsed=parsed,
            required_fields=["high", "low", "close", "quote_asset_volume"],
            feature_role="regime_feature",
            hypothesis_id="H02",
        ...
        add(_base_spec(
            ...
            inputs=["high_i", "low_i", "close_i", "quote_asset_volume_i"],
            parameters={"refresh_window_bars": 32, "range_window_bars": 20, "refresh_cap": 0.20},
            parsed=parsed,
            required_fields=["high", "low", "close", "quote_asset_volume"],
            feature_role="risk_feature",
            hypothesis_id="H03",
